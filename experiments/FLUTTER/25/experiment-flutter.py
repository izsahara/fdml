...
class Config1(Config):
    ...
    def __call__(self, X_train, Y_train):
        ...
        # Layer 1
        node11 = GPNode(kernel=SquaredExponential(length_scale=np.ones(nftr), variance=1.0), solver=LBFGSB(verbosity=0))
        node12 = GPNode(kernel=SquaredExponential(length_scale=np.ones(nftr), variance=1.0), solver=LBFGSB(verbosity=0))
        ...
        node11.kernel.length_scale.bounds = (1e-1 * np.ones(nftr), 10.0 * np.ones(nftr))
        node12.kernel.length_scale.bounds = (1e-1 * np.ones(nftr), 10.0 * np.ones(nftr))
        ...
        node21.kernel.length_scale.bounds = (1e-1 * np.ones(nftr), 10.0 * np.ones(nftr))
        node22.kernel.length_scale.bounds = (1e-1 * np.ones(nftr), 10.0 * np.ones(nftr))


        node31 = GPNode(kernel=SquaredExponential(length_scale=np.ones(nftr), variance=1.0), solver=LBFGSB(verbosity=0))
        node31.likelihood_variance.fix()
        node31.kernel.length_scale.bounds = (1e-1 * np.ones(nftr), 10.0 * np.ones(nftr))
...
def run_experiment(config : Config, n_thread):
    X_train = np.loadtxt("25-X_train.dat")
    Y_train = np.loadtxt("25-Y_train.dat")
    ...
    X_test = np.loadtxt("25-X_test.dat")
    Y_test = np.loadtxt("25-Y_test.dat")
    ...
    var = var.reshape(-1, 1)
    np.savetxt(f"Z{config.name}.dat", np.hstack([mean, var]), delimiter='\t')
    modelrmse = rmse(Y_test.ravel(), mean.ravel())
    print(f"RMSE = {modelrmse}")
    file = open(f"rmse.dat", "a")
...
if __name__ == "__main__":
    for ii in range(26, 52):
        print(f" ================== EXP {ii} ================== ")
        run_experiment(Config1(f"{ii}"), n_thread=5)
