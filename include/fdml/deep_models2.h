...
namespace fdml::deep_models2 {

	namespace gaussian_process {
		using namespace fdml::kernels;
		using namespace fdml::utilities;
		using namespace fdml::base_models2::gaussian_process::LBFGSB;
		using fdml::utilities::sobol::generate_sobol;
		using fdml::base_models2::models::GP;
		using fdml::base_models2::optimizer::Solver;
		using fdml::base_models2::optimizer::PSO;
		using fdml::base_models2::optimizer::SolverSettings;
		using fdml::base_models2::optimizer::OptData;
		...
		public:
			...
			GPNode(shared_ptr<Kernel> kernel, shared_ptr<Solver> solver) : GP(kernel, solver) {
				if (kernel->variance.value() != 1.0) { kernel->variance = 1.0; }
				if (!kernel->variance.fixed()) { kernel->variance.fix(); }
			}
			GPNode(shared_ptr<Kernel> kernel, const double& likelihood_variance, const double& scale_) :
				GP(kernel, likelihood_variance), scale("scale", scale_) {
				if (kernel->variance.value() != 1.0) { kernel->variance = 1.0; }
				if (!kernel->variance.fixed()) { kernel->variance.fix(); }
			}
			GPNode(shared_ptr<Kernel> kernel, const double& likelihood_variance, const double& scale_, shared_ptr<Solver> solver) :
				GP(kernel, solver, likelihood_variance), scale("scale", scale_) {
				if (kernel->variance.value() != 1.0) { kernel->variance = 1.0; }
				if (!kernel->variance.fixed()) { kernel->variance.fix(); }
			}
			GPNode(shared_ptr<Kernel> kernel, const Parameter<double>& likelihood_variance, const Parameter<double>& scale) :
		...
		public:
			...
			Layer(const std::vector<GPNode>& nodes_, bool initialize = true) : nodes(nodes_) {
				if (initialize) {
					// Checks
					for (std::vector<GPNode>::iterator node = nodes.begin(); node != nodes.end(); ++node) {
						if (node->inputs.size() > 0)
