	...
	namespace gaussian_process {
		...
		*/
		...
		public:
			...
			GPNode(shared_ptr<Kernel> kernel, const double& likelihood_variance, const double& scale_) : GP(kernel, likelihood_variance), scale("scale", scale_) {
				if (kernel->variance.value() != 1.0) { kernel->variance = 1.0; }
				if (!kernel->variance.fixed()) { kernel->variance.fix(); }
			}
			GPNode(shared_ptr<Kernel> kernel, const Parameter<double>& likelihood_variance, const Parameter<double>& scale) : GP(kernel, likelihood_variance), scale(scale) {
				if (kernel->variance.value() != 1.0) { kernel->variance = 1.0; }
				if (!kernel->variance.fixed()) { kernel->variance.fix(); }
			}
