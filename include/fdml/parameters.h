...
namespace fdml::parameters {
    ...
    public:        
        ...
        Parameter(std::string name, double value)  {
            ...
            _value = value;
            _transform = "none";
            _bounds = std::make_pair(0.001, 2.0);
        }
    ...
    public:
        ...
        Parameter(std::string name, TVector value) {
            ...
            for (int i = 0; i < _size; ++i) { lower_bound[i] = 0.001; }
            for (int i = 0; i < _size; ++i) { upper_bound[i] = 2.0; }
    ...
    /* Parameter Definition using constexpr, wont work with pybind11? */
    //template<typename T>
    //class Parameter : public BaseParameter<T> {
    //    Parameter() : BaseParameter() {}
    //    Parameter(std::string name, T value) : BaseParameter(name, value) { if constexpr (T == TVector) { _size = value.size(); } else { _size = 1; } }
    //    Parameter(std::string name, T value, std::tuple<double, double> bounds) : BaseParameter(name, value, bounds) { if constexpr (T == TVector) { _size = value.size(); } else { _size = 1; } }
    //    Parameter(std::string name, T value, std::string transform) : BaseParameter(name, value, transform) { if constexpr (T == TVector) { _size = value.size(); } else { _size = 1; } }
    //    Parameter(std::string name, T value, std::string transform, std::tuple<double, double> bounds) : BaseParameter(name, value, transform, bounds) { if constexpr (T == TVector) { _size = value.size(); } else { _size = 1; } }
    //    T lower_bound(bool transformed) override { 
    //        if constexpr (T == TVector)  {
    //            if (transformed) { return TVector::Constant(_size, 1, transform_fxn(_lower_bd, _transform)); }
    //            else { return TVector::Constant(_size, 1, _lower_bd); }
    //        }            
    //        else  {
    //            if (transformed) { return transform_fxn(_lower_bd, _transform); } 
    //            else { return _lower_bd; } 
    //        }
    //    }
    //    T upper_bound(bool transformed) override {
    //        if constexpr (T == TVector) {
    //            if (transformed) { return TVector::Constant(_size, 1, transform_fxn(_upper_bd, _transform)); }
    //            else { return TVector::Constant(_size, 1, _upper_bd); }
    //        }
    //        else {
    //            if (transformed) { return transform_fxn(_upper_bd, _transform); } 
    //            else { return _upper_bd; } 
    //        }
    //    }
    //};
