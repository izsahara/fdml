...
namespace fdml::kernels {
	...
	public:
		...
		Kernel(Parameter<TVector>& length_scale, Parameter<double>& variance) : length_scale(length_scale), variance(variance) {}
	...
	public: 
		...
		Stationary(Parameter<TVector>& length_scale, Parameter<double>& variance) : Kernel(length_scale, variance) {}
	...
	public:
		...
		SquaredExponential(Parameter<TVector>& length_scale, Parameter<double>& variance) : Stationary(length_scale, variance) {}
	...
	public:
		...
		Matern32(Parameter<TVector>& length_scale, Parameter<double>& variance) : Stationary(length_scale, variance) {}
	...
	public:
		...
		Matern52(Parameter<TVector>& length_scale, Parameter<double>& variance) : Stationary(length_scale, variance) {}
		...
		void gradients(const TMatrix& X, const TMatrix& dNLL, const TMatrix& R, const TMatrix& K, std::vector<double>& grad) override
		{
			if (!(*length_scale.is_fixed)) {
				...
				TMatrix dK_dR = (variance.value() * ((10 / 3) * R.array() - 5 * R.array() - (5 * sqrt(5) / 3) * square(R.array())) 
