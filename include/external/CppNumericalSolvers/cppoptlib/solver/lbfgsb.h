...
namespace cppoptlib::solver {
    ...
    private:
        ...
        function_state_t OptimizationStep(const function_t &function, const function_state_t &current, const state_t & /*state*/) override 
        {
            //const vector_t upper_bound_ = current.x.array() * 0 + std::numeric_limits<scalar_t>::max();
            //const vector_t lower_bound_ = current.x.array() * 0 + std::numeric_limits<scalar_t>::lowest();
            ...
            // sub-problem
            const vector_t subspace_min = SubspaceMinimization(current, cauchy_point, c);

            // STEP 4: perform linesearch and STEP 5: compute gradient
            scalar_t alpha_init = 1.0;
            const scalar_t rate = linesearch::MoreThuente<function_t, 1>::Search(current.x, subspace_min - current.x, function, alpha_init);
            ...
            const vector_t clipped_x_next = x_next.cwiseMin(upper_bound_).cwiseMax(lower_bound_);
        ...
        void SetLowerBound(const vector_t& lb) { lower_bound_ = lb; }
        void SetUpperBound(const vector_t& ub) { upper_bound_ = ub; }
        ...
        private:
            ...
            */
            ...
            void GetGeneralizedCauchyPoint(const function_state_t &current, vector_t *x_cauchy, dyn_vector_t *c) 
            {
            ...
            */
            scalar_t FindAlpha(const vector_t &x_cp, const dyn_vector_t &du, const std::vector<int> &free_variables) 
            {
            ...
            vector_t SubspaceMinimization(const function_state_t &current, const vector_t &x_cauchy, const dyn_vector_t &c) {
        ...
        private:
            ...
            vector_t lower_bound_ = vector_t::Zero(1);
            vector_t upper_bound_ = vector_t::Zero(1);
