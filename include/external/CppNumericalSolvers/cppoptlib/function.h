...
namespace cppoptlib::function {
    ...
    struct State {
        ...
        State(const int dim, const int order) : dim(dim), order(order), x(vector_t::Zero(dim)), gradient(vector_t::Zero(dim)) 
        { if (order > 1) { hessian = std::optional<matrix_t>(matrix_t::Zero(dim, dim)); } }
    ...
    public:
        static inline const int Dim = TDim;
    ...
    public:
        ...
        virtual State<scalar_t, vector_t, hessian_t> Eval(const vector_t &x, const int order = 2) const {
