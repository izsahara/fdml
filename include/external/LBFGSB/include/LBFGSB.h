...
private:
    ...
    std::vector<Vector>        p_hist;   // Parameter history
    std::vector<double>        f_hist;   // Parameter history
...
public:
    ...
    inline std::pair<double, Vector> minimize(Foo& f, Vector& x, Scalar& fx, const Vector& lb, const Vector& ub)
    {
        using std::abs;
        ...
        for( ; ; )
        {
            ...
            if(projgnorm <= m_param.epsilon || projgnorm <= m_param.epsilon_rel * x.norm())
            {
                if (k == 1) {
                    ...
                    min_idx = std::min_element(f_hist.begin(), f_hist.end()) - f_hist.begin();
                    min_f = *std::min_element(f_hist.begin(), f_hist.end());
                    return std::make_pair(min_f, p_hist[min_idx]);
                }
            }
    ...
    std::vector<Vector> get_history() { return p_hist; }
