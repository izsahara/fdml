...
void wrap_deep(py::module module) {
    ...
    using namespace fdml::deep_models2::gaussian_process;
    ...
    mNode
        ...
        .def(py::init<shared_ptr<Kernel>, const double&, const double&, shared_ptr<Solver>>(), py::arg("kernel"), py::arg("likelihood_variance"), py::arg("scale"), py::arg("solver"))
        .def(py::init<shared_ptr<Kernel>, const Parameter<double>&, const Parameter<double>&, shared_ptr<Solver>>(), py::arg("kernel"), py::arg("likelihood_variance"), py::arg("scale"), py::arg("solver"))
        .def("train", &GPNode::train)
        .def("gradients", &GPNode::gradients)
        .def("log_likelihood", &GPNode::log_likelihood)
        .def("log_marginal_likelihood", &GPNode::log_marginal_likelihood)
        ...
        .def_readwrite("solver", &GPNode::solver)
        ...
        .def(py::pickle(
            [/*__getstate__*/](const GPNode& p) {
                return py::make_tuple(p.kernel, p.likelihood_variance, p.scale, p.solver,
                    p.inputs, p.outputs,
                    p.missing,
            ...
            [/*__setstate__*/](py::tuple t) {
                ...
                GPNode p = GPNode(
                                      t[0].cast<shared_ptr<Kernel>>(),
                                      t[1].cast<Parameter<double>>(),
                                      t[2].cast<Parameter<double>>(),
                                      t[3].cast<shared_ptr<Solver>>()
                                 );
                p.inputs            = t[4].cast<TMatrix>();
                p.outputs           = t[5].cast<TMatrix>();
...
PYBIND11_MODULE(_deep_models2, module) {
